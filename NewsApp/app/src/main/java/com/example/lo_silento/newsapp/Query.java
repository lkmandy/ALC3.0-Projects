...
public final class Query {
    ...
    public static List<NewsCustom> fetchNewsData(String stringUrl) {
        if (stringUrl == null) {
            return null;
        }
        ...
        List<NewsCustom> newsList = extractDataFromJson(jsonResponse);
    ...
    private static List<NewsCustom> extractDataFromJson(String jsonResponse) {

        List<NewsCustom> newsList = new ArrayList<>();
        try {
            ...
            for (int i = 0; i < results.length(); i++) {
                ...
                NewsCustom newsObject = new NewsCustom(title, section, author, date, url);
